...
class SellLongOrder(BaseOrder):
    def __init__(self, position, amount, buy_on='open', stop=None,
                 limit=None, order_id=None):
        constraints = []
        #TODO: Stop and Limit orders should probably take "buy_on" into account
        if stop is not None:
        ...
        arguments = {'amount': amount, 'position': self.position}
        super().__init__(candle_feed=self.position.candle_feed,
                         constraints=constraints,
                         order_id=order_id,
                         command='sell',
                         arguments=arguments
    ...
    @sell_on.setter
    def sell_on(self, sell_on):
        if not isinstance(sell_on, str):
            raise TypeError
        sell_on = sell_on.lower()
        if not sell_on in ['high', 'low', 'open', 'close', 'mean']:
            raise ValueError
    ...
    def evaluate(self, dateindex=None):
        ...
        candle = self.candle_feed[dateindex]
        price = None
        if self.sell_on == 'high':
            price = candle.high
        elif self.sell_on == 'low':
            price = candle.low
        elif self.sell_on == 'open':
            price = candle.open
        elif self.sell_on == 'close':
            price = candle.close
        elif self.sell_on == 'mean':
            price = (candle.high + candle.low) / 2
        else:
            raise RuntimeError
        self.arguments['price'] = price
...
class SellShortOrder(BaseOrder):
    def __init__(self, position, amount, execute_on='open', stop=None,
                 limit=None, order_id=None):
        constraints = []
        #TODO: Stop and Limit orders should probably take "buy_on" into account
        if stop is not None:
